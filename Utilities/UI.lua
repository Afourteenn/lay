...
local function GetConfigs()
    if not isfolder("Lay") then makefolder("Lay") end
    if not isfolder("Lay\\Configs") then makefolder("Lay\\Configs") end
    if not isfile("Lay\\DefaultConfig.txt") then writefile("Lay\\DefaultConfig.txt","") end

    local Configs = {}
    for Index,File in pairs(listfiles("Lay\\Configs") or {}) do
        File = File:gsub("Lay\\Configs\\","")
        File = File:gsub(".json","")
        Configs[Index] = File
    end
    return Configs
...
local function ConfigsToList()
    ...
    for Index,File in pairs(listfiles("Lay\\Configs") or {}) do
        File = File:gsub("Lay\\Configs\\","")
        File = File:gsub(".json","")
        local DefaultConfig = readfile("Lay\\DefaultConfig.txt")
...
local function InitWindow(ScreenAsset,Window)
    ...
    function Window:SaveConfig(Name)
        ...
        if table.find(GetConfigs(),Name) then
            Config = HttpService:JSONDecode(readfile("Lay\\Configs\\"..Name..".json"))
        end
        for Index,Element in pairs(Window.Elements) do
        ...
        writefile("Lay\\Configs\\"..Name..".json",HttpService:JSONEncode(Config))
    end
    function Window:LoadConfig(Name)
        if table.find(GetConfigs(),Name) then
            local DecodedJSON = HttpService:JSONDecode(readfile("Lay\\Configs\\"..Name..".json"))
            for Index,Element in pairs(Window.Elements) do
            ...
            local ConfigSection = Tab:Section({Name = "Configs",Side = Side}) do
                ...
                ConfigSection:Button({Name = "Set",Callback = function()
                    if ConfigDropdown.Value and ConfigDropdown.Value[1] then
                        ...
                        writefile("Lay\\DefaultConfig.txt",DefaultConfig)
                        ConfigDivider:SetText(
                        "Default Config\n<font color=\"rgb(189,189,189)\">[ "..DefaultConfig.." ]</font>")
                    end
                end})
                ConfigSection:Button({Name = "Clear",Callback = function()
                    writefile("Lay\\DefaultConfig.txt","")
